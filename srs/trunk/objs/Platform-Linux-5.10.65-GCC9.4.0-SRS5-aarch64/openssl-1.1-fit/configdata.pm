...
our %config = (
  ...
  openssldir => "",
  options => "--prefix=/home/mmc/srs/trunk/objs/Platform-Linux-5.10.65-GCC9.4.0-SRS5-aarch64/openssl-1.1-fit/_release -DOPENSSL_NO_HEARTBEATS no-asan no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-shared no-ssl-trace no-ssl3 no-ssl3-method no-threads no-ubsan no-unit-test no-weak-ssl-ciphers no-zlib no-zlib-dynamic",
  perl_archname => "aarch64-linux-gnu-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.30.0",
  perlargv => [ "linux-aarch64", "--prefix=/home/mmc/srs/trunk/objs/Platform-Linux-5.10.65-GCC9.4.0-SRS5-aarch64/openssl-1.1-fit/_release", "-no-shared", "-no-threads", "-DOPENSSL_NO_HEARTBEATS" ],
  perlenv => {
      "AR" => undef,
      "BUILDFILE" => undef,
      "CC" => undef,
  ...
  prefix => "/home/mmc/srs/trunk/objs/Platform-Linux-5.10.65-GCC9.4.0-SRS5-aarch64/openssl-1.1-fit/_release",
  processor => "",
